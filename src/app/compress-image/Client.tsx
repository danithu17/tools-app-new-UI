...
import { imCompress } from "@/lib/imagemagick-utils";
...
const INPUT_ID = "compress-image-file-input";
...
export default function CompressImageClient() {
  ...
  const run = async () => {
    ...
    setProcessing(true);
    try {
      const out = await imCompress(bytes, format, quality);
      ...
      a.href = url;
      a.download = `${fileName.replace(/\.[^.]+$/, "")}-compressed.${ext}`;
      a.click();
  ...
  const savings = resultSize !== null ? Math.round((1 - resultSize / fileSize) * 100) : null;
    ...
    <main className="flex-1 max-w-3xl mx-auto w-full px-4 sm:px-6 py-10">
      ...
      {done && resultSize !== null && (
        <div className="mt-4 flex items-start gap-2 rounded-xl bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-900 px-4 py-3 text-sm text-green-700 dark:text-green-400">
          <span>✅</span>
          <span>
            Compressed! {formatSize(fileSize)} → {formatSize(resultSize)}
            {savings !== null && savings > 0 && ` (${savings}% smaller)`}
            {savings !== null && savings <= 0 && " (already optimized)"}
          </span>
        </div>
      )}
        ...
        <div className="mt-6 space-y-4">
          ...
          <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-slate-400 dark:text-slate-500">Original: {formatSize(fileSize)}</p>
            </div>
            <button type="button" onClick={() => { setBytes(null); setDone(false); setResultSize(null); if (previewUrl) URL.revokeObjectURL(previewUrl); setPreviewUrl(null); }}
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium mb-1.5">Quality: {quality}%</label>
            ...
            <div>
              ...
              <select value={format} onChange={(e) => setFormat(e.target.value as "jpg" | "png" | "webp")}
          ...
          >
            {processing ? (
              <><svg className="animate-spin w-4 h-4" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z" /></svg>Compressing…</>
            ) : <>🗜️ Compress &amp; Download</>}
          </button>
        </div>
      )}
