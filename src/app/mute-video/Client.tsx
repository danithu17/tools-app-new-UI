...
import { ffMuteVideo, makeBlob, triggerDownload } from "@/lib/ffmpeg-utils";

const INPUT_ID = "mute-video-input";
...
export default function MuteVideoClient() {
  ...
  const run = async () => {
    ...
    try {
      const out = await ffMuteVideo(file, setProgress);
      triggerDownload(makeBlob(out, "video/mp4"), file.name.replace(/\.[^.]+$/, "-muted.mp4"));
      setDone(true);
    } catch (e) { setError(e instanceof Error ? e.message : "Muting failed."); }
  ...
  return (
    <main className="flex-1 max-w-3xl mx-auto w-full px-4 sm:px-6 py-10">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2"><span className="text-3xl">🔇</span><h1 className="text-2xl sm:text-3xl font-black">Mute Video</h1></div>
        <p className="text-slate-500 dark:text-slate-400">Remove the audio track from your video in one click. Runs entirely in your browser.</p>
      </div>
      ...
      {done && <div className="mt-4 flex gap-2 rounded-xl bg-green-50 dark:bg-green-950/30 border border-green-200 px-4 py-3 text-sm text-green-700 dark:text-green-400"><span>✅</span><span>Audio removed — muted video downloaded!</span></div>}
      ...
      {file && (
        <div className="mt-6 space-y-5">
          ...
          <div className="rounded-xl border border-slate-200 dark:border-slate-700 px-4 py-4 bg-slate-50 dark:bg-slate-900 text-sm text-slate-600 dark:text-slate-400">
            🔇 The audio track will be permanently removed. The video picture quality stays untouched.
          </div>
          ...
          <button type="button" onClick={run} disabled={processing}
            ...
            {processing ? "Processing…" : "🔇 Mute & Download"}
          </button>
        </div>
      )}
      <ProcessingOverlay active={processing} label="Removing audio…" />
    </main>
  );
}
