...
const INPUT_ID = "pdf-grayscale-file-input";
...
export default function PdfGrayscaleClient() {
  ...
  const convert = async () => {
    ...
    setProcessing(true);
    try {
      const out = await mupdfToGrayscale(bytes);
      setOutputSize(out.byteLength);
      ...
      a.href = url;
      a.download = `${fileName.replace(/\.pdf$/i, "")}-grayscale.pdf`;
      a.click();
    ...
    <main className="flex-1 max-w-3xl mx-auto w-full px-4 sm:px-6 py-10">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          ...
          <h1 className="text-2xl sm:text-3xl font-black">PDF Grayscale</h1>
        </div>
        <p className="text-slate-500 dark:text-slate-400">
          Convert a color PDF to black &amp; white. Each page is rendered at 150 dpi grayscale. Runs entirely in your browser.
        </p>
      </div>
      ...
      {done && outputSize !== null && (
        <div className="mt-4 flex items-start gap-2 rounded-xl bg-green-50 dark:bg-green-950/30 border border-green-200 dark:border-green-900 px-4 py-3 text-sm text-green-700 dark:text-green-400">
          <span>✅</span>
          <span>Converted! {formatSize(fileSize)} → {formatSize(outputSize)}</span>
        </div>
      )}
      ...
      {bytes && (
        <div className="mt-6 space-y-4">
          <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
            ...
            <button type="button" onClick={() => { setBytes(null); setDone(false); setOutputSize(null); }}
