...
import { ffRotateVideo, makeBlob, triggerDownload } from "@/lib/ffmpeg-utils";

const INPUT_ID = "rotate-video-input";
const OPTIONS: { label: string; value: "90" | "180" | "270" | "fliph" | "flipv" }[] = [
  { label: "↩ 90° Left",   value: "270"   },
  { label: "↪ 90° Right",  value: "90"    },
  { label: "🔄 180°",      value: "180"   },
  { label: "↔ Flip H",     value: "fliph" },
  { label: "↕ Flip V",     value: "flipv" },
];

export default function RotateVideoClient() {
  const [file, setFile] = useState<File | null>(null);
  const [rotation, setRotation] = useState<"90" | "180" | "270" | "fliph" | "flipv">("90");
  ...
  const run = async () => {
    ...
    try {
      const out = await ffRotateVideo(file, rotation, setProgress);
      triggerDownload(makeBlob(out, "video/mp4"), file.name.replace(/\.[^.]+$/, `-${rotation}.mp4`));
      setDone(true);
  ...
  return (
    <main className="flex-1 max-w-3xl mx-auto w-full px-4 sm:px-6 py-10">
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2"><span className="text-3xl">🔄</span><h1 className="text-2xl sm:text-3xl font-black">Rotate Video</h1></div>
        <p className="text-slate-500 dark:text-slate-400">Rotate or flip your video — 90°, 180°, or mirror. Runs entirely in your browser.</p>
      </div>
      ...
      {done && <div className="mt-4 flex gap-2 rounded-xl bg-green-50 dark:bg-green-950/30 border border-green-200 px-4 py-3 text-sm text-green-700 dark:text-green-400"><span>✅</span><span>Video rotated and downloaded!</span></div>}
      ...
      {file && (
        <div className="mt-6 space-y-5">
          ...
          <div className="space-y-2">
            <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300">Transformation</label>
            <div className="grid grid-cols-3 sm:grid-cols-5 gap-2">
              {OPTIONS.map((o) => (
                <button key={o.value} type="button" onClick={() => setRotation(o.value)}
                  className={`py-2 px-1 rounded-lg text-xs font-semibold border transition-colors ${rotation === o.value ? "bg-red-600 text-white border-red-600" : "border-slate-200 dark:border-slate-700 hover:border-red-300"}`}>
                  {o.label}
                </button>
              ))}
            </div>
          ...
          {processing && (
            <div className="rounded-xl border border-slate-200 dark:border-slate-700 px-4 py-3 bg-slate-50 dark:bg-slate-900">
              <div className="flex justify-between text-xs text-slate-500 mb-1"><span>Rotating…</span><span>{Math.round(progress * 100)}%</span></div>
      ...
      <ProcessingOverlay active={processing} label="Rotating video…" />
    </main>
  );
}
